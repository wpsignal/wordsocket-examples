...
A block that renders a grid of published posts and keeps it alive: when a post is
published, updated, or removed, every browser viewing the page reflects the
change instantly. Cards slide into their new positions, flash green when added or
updated, and fade out red when removed. No reload, no polling.
...
- **`inc/wordsocket.php`** registers a `livingposts` channel and two triggers:
  `livingposts.updated` (on `save_post`) and `livingposts.deleted` (on
...
- **`src/living-posts.js`** listens for the two events, merges or removes the
  post in the store, re-sorts with the same ordering, and animates the reflow
  with a measure-based FLIP (`getBoundingClientRect` + the Web Animations API).
  Because it measures real pixels, it works in any column count and respects
